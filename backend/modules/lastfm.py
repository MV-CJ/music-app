...
def prefetch_next_page(page, limit):
    try:
        ...
        data = fetch_lastfm("chart.getTopTracks", {
            "page": str(next_page),
            "limit": str(limit)
        })

        processed = process_tracks(data, next_page, limit)
        set_cache(key, processed)

    except:
        pass  # silencioso mesmo
...
def process_tracks(data, page, limit):
    ...
    total = int(attr.get("total", 0)) if attr.get("total") else 1000
    ...
    for index, t in enumerate(tracks_raw):
        result.append({
            ...
            "yt_search": f"{t['name']} {t['artist']['name']}"
        })
...
def fetch_lastfm(method: str, params: dict | None = None):
    try:
        if params is None:
            params = {}
        ...
        print("🔥 ERRO LASTFM:", e)  # 👈 IMPORTANTE
        raise
...
def get_top_track():
    data = fetch_lastfm("chart.getTopTracks", {"limit": "1"})

    tracks = normalize_tracks(data.get("tracks", {}).get("track"))
...
def get_top_tracks(page: int = 1, limit: int = 12):
    ...
    try:
        ...
        processed = process_tracks(data, page, limit)
        ...
        # 🔥 3. fallback (cache antigo)
        if cached:
            return cached

        raise HTTPException(
            status_code=500,
            detail="Erro ao buscar dados e sem cache disponível"
        )
